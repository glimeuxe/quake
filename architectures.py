...
# New units
class SomnialUnit4(nn.Module):  # SomnialUnit2 but with FNO
	...
	def forward(self, x_t):
		...
		g = self.blender(x_s_hat, x_t)
		return g * x_s_hat + (1 - g) * x_t

class SomnialUnit3(nn.Module):  # SomnialUnit2 but with epsilon noise
	def __init__(self, in_channels, k=10, epsilon=0.01):
		super().__init__()
		self.M = deque(maxlen=k)
		self.generator = nn.Sequential(
			nn.Conv2d(in_channels, in_channels, kernel_size=1),
			nn.ModuleList([nn.Identity()])[0]
		)
		self.generator[1].forward = lambda x: x + epsilon * torch.randn_like(x)
...
class SomnialUnit2(nn.Module):  # SomnialUnit but with modified spatial attention
	def __init__(self, in_channels, k=10):
		super().__init__()
		self.M = deque(maxlen=k)
		self.generator = nn.Sequential(
...
class S3R50ViTB16(nn.Module):
	def __init__(self):
		...
		self.somnial = SomnialUnit3(in_channels)
